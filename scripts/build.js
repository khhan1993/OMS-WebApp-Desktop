...
let FileSizeReporter = require('react-dev-utils/FileSizeReporter');
let measureFileSizesBeforeBuild = FileSizeReporter.measureFileSizesBeforeBuild;
let printFileSizesAfterBuild = FileSizeReporter.printFileSizesAfterBuild;

let useYarn = fs.existsSync(paths.yarnLockFile);
...
function build(previousFileSizes) {
  ...
  webpack(config).run((err, stats) => {
    ...
    if (publicUrl && publicUrl.indexOf('.github.io/') !== -1) {
      ...
      let build = path.relative(process.cwd(), paths.appBuild);
      console.log('The ' + chalk.cyan(build) + ' folder is ready to be deployed.');
